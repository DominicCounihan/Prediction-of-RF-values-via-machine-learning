...
print("Loading data...")
df = pd.read_csv('KP_DATA_test.csv', 
                 names=['smiles', 'CC#N','KP'])
print(df['smiles'])
...
def process_eluient():
     ...
     # Load data
    eluents = ['CC#N','O']
    df = pd.read_csv('KP_data_test.csv', names=['smiles', 'CC#N','O', 'KP'])
    ...
    el_data = np.array([
    pd.to_numeric(df['CC#N'], errors='coerce').values,
    np.ones(len(df)) - pd.to_numeric(df['CC#N'], errors='coerce').values])
    ...
    joblib.dump(matrix_properties, 'matrix_properties.save')
    ...
    for prop in range(len(eluents)):  # For each eluent (0 to 4)
...
def get_mol2vec_embeddings(mols, model, radius=1, unseen='UNK'):
    ...
    return embeddings
joblib.dump(get_mol2vec_embeddings,'mol2vec.save')
...
X_other = df.drop(['smiles','mol','KP','mol2vec','CC#N'], axis=1).values
print(X_other)
...
def build_model(input_shape):
    ...
    conv1 = layers.Conv1D(500, 2, activation='relu')(seq)
    ...
    combined = layers.concatenate([fcoutput1,mol_features])
...
plt.tight_layout()
plt.show()
joblib.dump(scaler, 'scalerKP.save')
model.save('model_KP.keras')
def predict_rf(smiles, eluent_percents):
    # Load model and scaler
    if os.path.exists('model_KP.keras'):
        model = tf.keras.models.load_model('model_RF_4.keras')
    else:
        raise FileNotFoundError("Model not found. Please train the model first.")
    
    if os.path.exists('scalerKP.save'):
        scaler = joblib.load('scalerKP.save')
    else:
...
smiles='COc1cc(c2c(c1)oc3cc(c(cc3c2=O)OC)OC)O'
eluients_percen=[0.4,0.6]
rf_value = predict_rf(smiles,eluients_percen)
print(f"Predicted KP: {rf_value:.4f}")
